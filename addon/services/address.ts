...
export default class AddressService extends Service {
  ...
  get shouldCache() : boolean {
    if(this.config.hasOwnProperty('ember-mist-components') &&
  ...
  * getCountrySelectOptions() : SelectOption[] {
    ...
    yield this.ajax.request(`${this.apiEndpoint}address/address/countries/selectoptions`)
    .then((response : any) => {
      countrySelectOptions = <SelectOption[]> response;
    })
    .catch((error : any) => {
      console.log(error);
      this.toast.error('Error fetching countries', 'Error fetching countries');
    });
  ...
  * getAddressFormat(countryCode : string) : any {
    ...
    yield this.ajax.request(`${this.apiEndpoint}address/address/format/${countryCode}`)
    .then((response: any) => {
      foundAddressFormat = response;
    })
    .catch((error : any) => {
      console.log(error);
      this.toast.error('Error fetching address format', 'Error fetching address format');
    });
  ...
  * getSubdivisionSelectOptions(parentGrouping: string) {
    ...
    yield this.ajax.request(`${this.apiEndpoint}address/address/subdivisions/${parentGrouping}`)
    .then((response: any) => {
    ...
    .catch((error : any) => {
      console.log(error);
      this.toast.error('Error fetching subdivisions', 'Error fetching subdivisions');
    });
