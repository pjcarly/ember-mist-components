 ...
 */
export default class JsonApiEmbeddedSerializer extends JSONAPISerializer.extend(EmbeddedRecordsMixin) {
  attrs: any;
  ...
  normalizeSaveResponse(store: Store, primaryModelClass: any, payload: any, id: string|number) {
    const attrs = this.attrs;
            ...
            if(!isBlank(localChildren)) {
              localChildren.toArray().forEach((localChild: DrupalModel) => {
                ...
                if(isBlank(childId) || isBlank(returnedIds) || !returnedIds.includes(childId)) {
                  debug(`(serializer) Unloading ${localChild.name} because ${isBlank(childId) ? 'id was blank': 'id wasn\'t returned in included hash'}`);
  ...
  normalizeDeleteRecordResponse(store: Store, primaryModelClass: any, _: any, id: string) {
    const attrs = this.attrs;
